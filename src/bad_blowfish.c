...
int BlowFish_init(struct BlowFish *bf)
{
	...
	for (i = 0; i < 255; i++) {
		for (j = i + 1; j < 256; j++) {
			if ((bf->S[0][i] == bf->S[0][j]) || (bf->S[1][i] == bf->S[1][j]) ||
					(bf->S[2][i] == bf->S[2][j]) || (bf->S[3][i] == bf->S[3][j])) {
				return false;
			}
		}
	}
...
int BlowFishSetKey(struct BlowFish *bf, void *key, int bitlength)
{
...
int CipherBlockChainSetKey(struct CipherBlockChain *cbc, void *key, int bitlength) {
	if (cbc->_cipher) {
		return BlowFishSetKey(cbc->_cipher, key, bitlength);
	}
	return false;
}
